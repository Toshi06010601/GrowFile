<div>
    <div x-data="{
        ...
        <input class="mt-1 block w-full rounded-md" x-model="search" placeholder="Search..." @input="updateShow()"
    ...
    @foreach ($selectedTags as $tag)
        <div
            class="inline-flex justify-around select-none min-w-12 align-middle px-1 py-1 m-1 rounded-md bg-black text-sm text-white cursor-pointer hover:bg-gray-700 transition-all duration-200">
                ...
                <input type="checkbox" class="hidden" wire:model="selectedTagIds" value={{ $tag->id }} id="selectedTag{{ $tag->id }}">
        </div>
    @endforeach
