...
class CourseEditor extends Component
{
    ...
    */
    ...
    public function setCourse($id)
    {
        try {
            ...
            $this->dispatch('flash-message', type: 'error', message: __('flash.course.not-found'));
            logger()->warning('Course not found', ['course_id' => $id]);
        } catch (Exception $e) {
            $this->dispatch('flash-message', type: 'error', message: __('flash.course.failed-load'));
    ...
    */
    private function finishAction(string $actionName): void
    {
        ...
        $this->dispatch('flash-message', type: 'success', message: __("flash.course.{$actionName}"));
    }

    private function handleError(string $actionName, Exception $e): void
    {
        $this->dispatch('flash-message', type: 'error', message: __("flash.course.failed-{$actionName}"));
