<nav x-data="{
            ...
            <div class="px-4 w-full flex items-center gap-4 md:gap-8">
                ...
                <div
                    ...
                    <a class="flex flex-col justify-center items-center"
                        href="{{ route('professional_profile.index') }}" wire:navigate.hover>
                ...
                @auth
                    ...
                    <div
                        ...
                        <a class="flex flex-col justify-end items-center hover:scale-105"
                            href={{ route('professional_profile.show', $userProfile ? $userProfile->slug : '') }} wire:navigate.hover>
                ...
                @else
                    ...
                    <div class="hidden lg:flex sm:items-center sm:ms-6 sm:gap-2">
                        <x-primary-button :href="route('register')" wire:navigate.hover class="hover:scale-105">Start Now</x-primary-button>
                        <x-secondary-button :href="route('login')" wire:navigate class="hover:scale-105">Login</x-secondary-button>
                    </div>
                @endauth
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="block md:hidden">
        <div class="pt-2 pb-3">
            <x-responsive-nav-link :href="route('home')" wire:navigate.hover class="flex flex-row gap-2 hover:bg-brand-secondary-300">
                <img src="{{ asset('/images/icons/house.svg') }}" alt="" class="block h-6 w-auto" />
                {{ __('Home') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('professional_profile.index')" wire:navigate.hover class="flex flex-row gap-2">
                <img src="{{ asset('/images/icons/people.svg') }}" alt="" class="block h-6 w-auto" />
                {{ __('Network') }}
            </x-responsive-nav-link>
            {{-- For Logged in user only --}}
            @auth
                <x-responsive-nav-link :href="route('professional_profile.show', $userProfile ? $userProfile->slug : '')" wire:navigate.hover class="flex flex-row gap-2">
        ...
        <div class="pt-2 pb-1 border-t border-brand-secondary-200">
            ...
            @else
                <div class="mt-2 flex flex-row justify-center gap-3">
                    <x-primary-button :href="route('register')" wire:navigate.hover>Start Now</x-primary-button>
                    <x-secondary-button :href="route('login')" wire:navigate.hover>Login</x-secondary-button>
                </div>
            @endauth
        </div>
    </div>
