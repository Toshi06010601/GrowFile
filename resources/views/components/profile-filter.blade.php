...
<div class="flex flex-col pt-7 sm:pt-0">
    ...
    <div>
        ...
        <form method="get" action="{{ route('professional_profile.index') }}">
            ...
            {{-- Name --}}
            <div class="mt-2">
                <label for="{{ $idPrefix }}-name" class="text-sm text-gray-600 block mb-1">Name</label>
                <x-text-input id="{{ $idPrefix }}-name" 
                              name="name" 
                              placeholder="Name" 
                              value="{{ old('name', $name) }}" 
                              class="text-sm leading-4 w-full" />
            </div>

            {{-- Location --}}
            <div class="mt-2">
                <label for="{{ $idPrefix }}-location" class="text-sm text-gray-600 block mb-1">Location</label>
