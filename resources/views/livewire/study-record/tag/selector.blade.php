    ...
    <div x-data="{
        ...
        <template x-for="tag in selectedTags()" :key="tag.id"
            ...
            <div
                ...
                <span x-text="tag.name"></span>
                <button type="button" class="font-bold ml-1 text-white bg-none border-none cursor-pointer text-sm leading-none"
                    x-on:click.prevent="selected = selected.filter(s => s != tag.id)" tabindex="-1">
                    &times;
                </button>
            </div>
        </template>
