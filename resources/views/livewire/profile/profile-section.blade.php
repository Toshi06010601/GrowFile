            ...
            <div class="mx-auto max-w-xs flex flex-col justify-around items-start">
                <div class="relative w-24 sm:w-32 mx-auto">
                    ...
                    <div class="size-20 sm:size-28 rounded-full overflow-hidden mx-auto border-2 border-white shadow-md">
                        <img src="{{ asset("/storage/{$this->profile->profile_image_path}") }}" alt="profile image"
                    ...
                    @can('update', $this->profile)
                        <x-section.edit-icon class="block absolute z-10 bottom-0 -right-1" x-data=""
                            x-on:click="
                $dispatch('set-profile', { id: {{ $this->profile->id }} });" />
                    @endcan
                </div>

                {{-- full name --}}
                <div class="mx-auto mt-2">
                    <h3 class="text-center text-lg leading-5 sm:text-2xl">
                        <strong>{{ $this->profile->full_name }}</strong></h3>
                </div>

                {{-- headline --}}
                <div class="mx-auto mt-2">
                    <p class="text-center text-base sm:text-xl text-brand-secondary-600 leading-none">
                        {{ $this->profile->headline }}</p>
                </div>
            </div>
            ...
            <div class="flex-1 min-w-[150px] flex flex-col justify-center items-center gap-1 px-1">
                ...
                <div class="sm:mx-auto mt-2">
                    <div class="text-white bg-green-900 rounded-full py-1 px-3 text-center">
                        {{ str_replace('_', ' ', $this->profile->job_status) }}</div>
                </div>
                ...
                <div class="mx-5 sm:mx-auto mt-2 flex flex-row justify-center items-start gap-0">
                    ...
                    <p class="text-brand-secondary-600 text-center leading-none">{{ $this->profile->location }}</p>
                </div>
                ...
                <div class="flex flex-row justify-center gap-3 mt-5 text-sm sm:text-base">
                    ...
                    <figure class="flex flex-col">
                        <a href={{ $this->profile->github_link }} target="_blank" class="hover:scale-105">
                    ...
                    {{-- LinkedIn --}}
                    <figure class="flex flex-col">
                        <a href={{ $this->profile->linkedin_link }} target="_blank" class="hover:scale-105">
                ...
                @auth
                    @if (Auth::id() !== $userId)
                        <livewire:Profile.Partials.FollowButton :userId="$userId" :isFollowing="!is_null($this->profile->user->authFollows)" idPrefix="laptop" />
                    @endif
                @endauth
            </div>
        @endif
